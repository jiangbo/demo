...
//
//    Cmdline:
//        sokol-shdc -i .\src\shader\texture.glsl -o .\src\shader\texture.glsl.zig -l glsl410:metal_macos:hlsl5:glsl300es:wgsl -f sokol_zig --reflection
//
//    Overview:
//    =========
...
//        Fragment Shader: fs
//        Attributes:
//            ATTR_texture_position0 => 0
//            ATTR_texture_color0 => 1
//            ATTR_texture_texcoord0 => 2
//    Bindings:
...
//            Bind slot: SMP_smp => 0
//
pub const ATTR_texture_position0 = 0;
pub const ATTR_texture_color0 = 1;
pub const ATTR_texture_texcoord0 = 2;
...
const fs_source_metal_macos = [439]u8 {
    ...
    0x34,0x20,0x75,0x76,0x20,0x5b,0x5b,0x75,0x73,0x65,0x72,0x28,0x6c,0x6f,0x63,0x6e,
...
pub fn textureAttrSlot(attr_name: []const u8) ?usize {
...
pub fn textureImageSlot(img_name: []const u8) ?usize {
    if (std.mem.eql(u8, img_name, "tex")) {
        return 0;
    }
    return null;
}
pub fn textureSamplerSlot(smp_name: []const u8) ?usize {
    if (std.mem.eql(u8, smp_name, "smp")) {
        return 0;
    }
    return null;
}
pub fn textureUniformblockSlot(ub_name: []const u8) ?usize {
    if (std.mem.eql(u8, ub_name, "vs_params")) {
        return 0;
    }
    return null;
}
pub fn textureUniformblockSize(ub_name: []const u8) ?usize {
    if (std.mem.eql(u8, ub_name, "vs_params")) {
        return @sizeOf(VsParams);
    }
    return null;
}
pub fn textureUniformOffset(ub_name: []const u8, u_name: []const u8) ?usize {
    if (std.mem.eql(u8, ub_name, "vs_params")) {
...
pub fn textureUniformDesc(ub_name: []const u8, u_name: []const u8) ?sg.GlslShaderUniform {
    if (std.mem.eql(u8, ub_name, "vs_params")) {
        if (std.mem.eql(u8, u_name, "viewMatrix")) {
...
pub fn textureStoragebufferSlot(sbuf_name: []const u8) ?usize {
    _ = sbuf_name;
    return null;
}
